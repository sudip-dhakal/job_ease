...
# from Matching import Matching  # Import the modified Matching function
...
#     # Fetch job description data from the JOBS collection
#     jd_doc = JOBS.find_one({"_id": ObjectId(job_id)}, {"FileData": 1})
#     if jd_doc is None:
#         return jsonify({"StatusCode": 400, "Message": "Job description not found"})
#     # Fetch resume data from the resumeFetchedData collection
#     resume_doc = resumeFetchedData.find_one({"UserId": ObjectId(session['user_id'])})
#     if resume_doc is None:
#         return jsonify({"StatusCode": 400, "Message": "Resume data not found"})

#     # Call the Matching function with the data
#     match_percentage = Matching(resume_doc, jd_doc)
